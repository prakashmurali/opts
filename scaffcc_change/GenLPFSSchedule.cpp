...
void GenLPFSSched::lpfs(Function* F, int ts, int simd_l, int refill_simd, int opp_simd){
  ...
  //errs() << "Prakash: In lpfs with function " << F->getName() << "\n";
...
void GenLPFSSched::PrintGateRequirements(){
  int cnt=0;
  for(map<Instruction*, op>::iterator mp = mapCalls.begin(); mp!=mapCalls.end(); ++mp){
    cnt++;
    Instruction *pInst = (*mp).first;
    CallInst *pCI = dyn_cast<CallInst>(pInst);
    Function *pCallee = pCI->getCalledFunction();

    if (pCallee->isIntrinsic()) {    
      errs() << "\n" << cnt << " " <<  pCallee->getName() << "\n" << cnt << " ";
      map<Instruction*, op>::iterator mp1 = mapCalls.find(pInst);
      for(int i=0; i < (*mp1).second.name.numArgs; ++i) {
        qArgInfo arg = (*mp1).second.name.args[i];
        stringstream ss;
        ss << arg.index;
          ...
          for (std::vector<CallGraphNode*>::const_iterator nsccI = nextSCC.begin(), E = nextSCC.end(); nsccI != E; ++nsccI) {
            ...
            if(F && !F->isDeclaration()){
              ...
              if(!(schedule.empty())) {
                ...
                if(FULL_SCHED){
                  //errs() << "Prakash: Printing schedule\n";
                  //print_schedule(F,op_count);
                  PrintGateRequirements();
                }
                if(MOVES_SCHED)
